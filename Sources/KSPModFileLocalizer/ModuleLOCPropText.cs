...
{
    class ModuleLOCPropText : InternalModule
    {
        //--------------------Public fields---------------------
        ...
        /// </summary>
        [KSPField]
        public string shader = "KSP/Diffuse";
        ...
        public void Start()
        {
            ...
            Shader usdeShader = null;
            for (int i = 0; i < shaders.Length; i++)
            {
                if (shaders[i].name == shader)
                {
                    usdeShader = shaders[i];
                    break;
                }
            }
            ...
            Transform screenTransform = internalProp.FindModelTransform(textQuadName);
            ...
            textMesh.material = new Material(Instantiate(usdeShader));
            //set the material for the mesh
            //Material tmpMat = textMesh.material;
            //textMesh.material.shader = shader;
            ...
            LCDScreen.transform.localPosition = new Vector3(0, 0, (M.bounds.size.z / 2) + 0.0001f);
            ...
            //textMesh.material = new Material(shader);//.shader = shader;
